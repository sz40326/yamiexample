...
export default class HealthBar {
    ...
    specialAttr;
    specialAttrValue;
    backgroundIdFriendly;
    foregroundIdFriendly;
    backgroundIdEnemy;
    foregroundIdEnemy;
    backgroundIdSpecial;
    foregroundIdSpecial;
    ...
    async onStart() {
        ...
        switch (this.display) {
            ...
            case 'auto-except-for-player-team':
                this.isHidden = actor => {
                    if (actor.teamId === Party.player?.teamId)
                        return false;
                    return Time.elapsed - actor.hitTimestamp > duration;
                };
                break;
        ...
        if (specialBarEnabled && specialAttr && specialAttrValue) {
            ...
            // 创建完整版的切换器
            this.switcher = (actor) => {
                if (actor.attributes[specialKey] === specialValue)
                    return 4;
                if (actor === Party.player || !Party.player)
                    return 0;
        ...
        for (let i = 0, y = 0; i < length; i++) {
            ...
            GL.texSubImage2D(GL.TEXTURE_2D, 0, 0, y, width, height, GL.RGBA, GL.UNSIGNED_BYTE, images[i]);
    ...
    render() {
        ...
        if (vi !== 0) {
            ...
            const matrix = Matrix.instance.project(gl.flip, Camera.width, Camera.height).translate(-Camera.scrollLeft, -Camera.scrollTop);
...
//# sourceMappingURL=%E7%94%9F%E5%91%BD%E5%80%BC%E6%9D%A1.f0ee6c16284dcf99.js.map