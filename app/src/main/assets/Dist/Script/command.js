...
/** ******************************** 指令函数列表 ******************************** */
class CommandFunctionList extends Array {
    type = '';
    path = '';
    default;
    enabled;
...
let Command = new class CommandCompiler {
     ...
     */
    compile(commands, callback, loop = false) {
    ...
    return({ type, value }) {
        ...
        const setter = type === 'none'
            ? Function.empty
            : (value) => {
                CurrentEvent.attributes[returnKey] = value;
            };
        switch (type) {
            case 'none':
                return Command.skip;
     ...
     * @returns 数值访问器函数
     */
    compileNumber(number, defValue, min, max) {
    ...
    /** 编译变量对象 */
    compileVariable = function compileVariable(IIFE) {
        const compilers = {
        ...
        return function (variable, operation) {
            ...
            switch (type) {
                case 'local':
                    return (value) => operation(CurrentEvent.attributes, key, value);
                case 'global':
                    return (value) => operation(Variable.map, key, value);
                case 'self':
                    return (value) => CurrentEvent.selfVarId && operation(SelfVariable.map, CurrentEvent.selfVarId, value);
                case 'actor':
                case 'skill':
                case 'state':
                case 'equipment':
    ...
    compilePosition(position) {
        switch (position.type) {
            ...
            case 'object': {
                ...
                return () => Scene.entity.get(objectId);
            }
            case 'mouse':
                return () => {
                    return {
    ...
    /** 编译条件列表 */
    compileConditions = function (IIFE) {
                    ...
                    switch (operation) {
                        ...
                        case 'in-screen':
                            return () => {
                                const actor = getActor();
                                if (actor) {
                                    return (actor.x >= Camera.scrollLeftT &&
     ...
     * @returns 文本内容解析函数
     */
    compileTextContent(content) {
        if (typeof content === 'object') {
            return Command.compileString(content);
        }
        // 获取变量标签正则表达式
        const method = this.compileTextContent;
        let { regexp } = method;
        if (!regexp) {
    ...
    /** 显示文本 */
    showText({ target, parameters, content }) {
    ...
    /** 显示选项 */
    showChoices({ choices, parameters }) {
        // 解析变量文本内容
        const method = this.showChoices;
        ...
        if (!parseVariable) {
            ...
            parseVariable = method.parseVariable = (contents) => contents.map(mapper);
        }
        const { commands, index } = this.stack.get();
    ...
    /** 编译事件参数访问器 */
    compileArgumentAccessors(eventArgs, eventParams) {
        // 编译每个参数写入和读取器
        const flags = {};
        const getters = [];
        const setters = [];
        outer: for (const { type, key } of eventParams) {
            for (const arg of eventArgs) {
                if (arg.key === key && arg.type === type) {
                    ...
                    flags[key] = true;
                    const variable = { type: 'local', key: key };
                    ...
                    switch (type) {
                        case 'boolean':
                            ...
                            break;
                        case 'number':
                            getters.push(Command.compileNumber(value));
                            setters.push(Command.compileVariable(variable, Attribute.NUMBER_SET));
                            break;
                        case 'string':
                            getters.push(Command.compileString(value));
                            setters.push(Command.compileVariable(variable, Attribute.STRING_SET));
                            break;
                        case 'object':
                            getters.push(Command.compileVariable(value, Attribute.OBJECT_GET));
                            setters.push(Command.compileVariable(variable, Attribute.OBJECT_SET));
                            break;
                        case 'actor':
                            getters.push(Command.compileActor(value));
                            setters.push(Command.compileVariable(variable, Attribute.OBJECT_SET));
                            break;
                        case 'skill':
                            getters.push(Command.compileSkill(value));
                            setters.push(Command.compileVariable(variable, Attribute.OBJECT_SET));
                            break;
                        case 'state':
                            getters.push(Command.compileState(value));
                            setters.push(Command.compileVariable(variable, Attribute.OBJECT_SET));
                            break;
                        case 'equipment':
                            getters.push(Command.compileEquipment(value));
                            setters.push(Command.compileVariable(variable, Attribute.OBJECT_SET));
                            break;
                        case 'item':
                            getters.push(Command.compileItem(value));
                            setters.push(Command.compileVariable(variable, Attribute.OBJECT_SET));
                            break;
                        case 'trigger':
                            getters.push(Command.compileTrigger(value));
                            setters.push(Command.compileVariable(variable, Attribute.OBJECT_SET));
                            break;
                        case 'light':
                            getters.push(Command.compileLight(value));
                            setters.push(Command.compileVariable(variable, Attribute.OBJECT_SET));
                            break;
                        case 'element':
                            getters.push(Command.compileElement(value));
                            setters.push(Command.compileVariable(variable, Attribute.OBJECT_SET));
                            break;
                    }
                    continue outer;
    ...
    /** 编译事件返回值访问器 */
    compileReturnValueAccessors(eventResult, returnType) {
        ...
        switch (eventResult.type) {
            ...
            default: {
                const returnKey = this.returnKey;
                getReturnValue = () => {
                    result = CurrentEvent.attributes[returnKey];
                };
                break;
            }
        }
        ...
        switch (eventResult.type) {
            ...
            case 'boolean': {
                ...
                setReturnValue = () => { if (typeof result === 'boolean')
                    setter(result); };
                break;
            }
            case 'number': {
                const setter = Command.compileVariable(eventResult.variable, Attribute.NUMBER_SET);
                setReturnValue = () => { if (Number.isFinite(result))
                    setter(result); };
                break;
            }
            case 'string': {
                const setter = Command.compileVariable(eventResult.variable, Attribute.STRING_SET);
                setReturnValue = () => { if (typeof result === 'string')
                    setter(result); };
                break;
            }
            case 'object': {
                const setter = Command.compileVariable(eventResult.variable, Attribute.OBJECT_SET);
                setReturnValue = () => { if (typeof result === 'object' || result === undefined)
                    setter(result); };
                break;
            }
            case 'actor': {
                const setter = Command.compileVariable(eventResult.variable, Attribute.OBJECT_SET);
                setReturnValue = () => { if (result instanceof Actor || result === undefined)
                    setter(result); };
                break;
            }
            case 'skill': {
                const setter = Command.compileVariable(eventResult.variable, Attribute.OBJECT_SET);
                setReturnValue = () => { if (result instanceof Skill || result === undefined)
                    setter(result); };
                break;
            }
            case 'state': {
                const setter = Command.compileVariable(eventResult.variable, Attribute.OBJECT_SET);
                setReturnValue = () => { if (result instanceof State || result === undefined)
                    setter(result); };
                break;
            }
            case 'equipment': {
                const setter = Command.compileVariable(eventResult.variable, Attribute.OBJECT_SET);
                setReturnValue = () => { if (result instanceof Equipment || result === undefined)
                    setter(result); };
                break;
            }
            case 'item': {
                const setter = Command.compileVariable(eventResult.variable, Attribute.OBJECT_SET);
                setReturnValue = () => { if (result instanceof Item || result === undefined)
                    setter(result); };
                break;
            }
            case 'trigger': {
                const setter = Command.compileVariable(eventResult.variable, Attribute.OBJECT_SET);
                setReturnValue = () => { if (result instanceof Trigger || result === undefined)
                    setter(result); };
                break;
            }
            case 'light': {
                const setter = Command.compileVariable(eventResult.variable, Attribute.OBJECT_SET);
                setReturnValue = () => { if (result instanceof SceneLight || result === undefined)
                    setter(result); };
                break;
            }
            case 'element': {
                const setter = Command.compileVariable(eventResult.variable, Attribute.OBJECT_SET);
                setReturnValue = () => { if (result instanceof UIElement || result === undefined)
                    setter(result); };
                break;
            }
        }
     ...
     * @returns 事件指令列表
     */
    getInheritedCommandList(object, found = false) {
        let override;
        const event = CurrentEvent;
     ...
     * @returns 事件指令列表
     */
    getInheritedCommandList2(object) {
        let override;
        const event = CurrentEvent;
        const array = event[Command.inheritKey];
        if (array && array.length > 0) {
     ...
     * @returns 两个指令函数
     */
    compileCommandTupleInNamespace(commands) {
        const attrMapKey = Symbol('ATTRIBUTES_BACKUP');
        const fn1 = () => {
            CurrentEvent[attrMapKey] = CurrentEvent.attributes;
        ...
        const fn2 = () => {
            CurrentEvent.attributes = CurrentEvent[attrMapKey];
            return true;
        };
        return [fn1, fn2];
    }
    ...
    compileInheritedCommandTuple() {
        ...
        const fn1 = () => {
            ...
            else if (owner instanceof Skill ||
    ...
    /** 调用事件 */
    callEvent({ type, actor, skill, state, equipment, item, light, element, eventId, eventArgs, eventResult, eventType }) {
        switch (type) {
            case 'global': {
                // 补丁：2025-2-22
                ...
                const fn = () => {
                    ...
                    let fn1 = Command.skip;
                    let fn2 = Command.skip;
                    if (commands) {
    ...
    /** 停止正在执行的对象事件(通过类型) */
    stopActiveEventsByType(updaters, type) {
    ...
    /** 停止事件 */
    stopEvent({ type, actor, skill, state, equipment, item, light, element, eventId, eventType }) {
        // 补丁：2025-2-27
        if (type === undefined) {
            type = 'current';
        }
    ...
    /** 注册事件 */
    registerEvent({ target, actor, element, operation, type, priority, tag, commands }) {
        switch (target) {
            case 'global':
                switch (operation) {
                    case 'register': {
                        ...
                        return () => {
                            commandList.inheritance = CurrentEvent;
                            EventManager.register(getTag(), type, commandList);
                            return true;
                        };
                    }
                    case 'unregister': {
            ...
            case 'actor':
                switch (operation) {
                    case 'register': {
                        ...
                        return () => {
                            commandList.inheritance = CurrentEvent;
                            getActor()?.register(type, commandList);
                            return true;
                        };
                    }
                    case 'unregister': {
            ...
            case 'element':
                switch (operation) {
                    case 'register': {
                        ...
                        return () => {
                            commandList.inheritance = CurrentEvent;
                            getElement()?.register(type, commandList);
                            return true;
                        };
                    }
                    case 'unregister': {
    ...
    /** 设置事件 */
    setEvent({ operation, variable, eventId, choiceIndex }) {
        switch (operation) {
    ...
    /** 过渡 */
    transition({ variable, start, end, easingId, duration, commands }) {
        if (commands.length === 0) {
            return null;
        }
        const context = Command.stack.get();
    ...
    /** 创建元素 */
    createElement({ operation, parent, uiId, presetId }) {
        switch (operation) {
    ...
    /** 设置图像 */
    setImage({ element, properties }) {
        const method = this.setImage;
        if (!method.clipMap) {
            method.clipMap = {
                'clip-0': true,
                'clip-1': true,
    ...
    /** 加载图像 */
    loadImage({ element, type, actor, skill, state, equipment, item, key, variable }) {
        const method = this.loadImage;
        let { setImageClip } = method;
        if (!setImageClip) {
            setImageClip = method.setImageClip = (element, object) => {
    ...
    /** 改变图像色调 */
    tintImage({ element, mode, tint, easingId, duration, wait }) {
        const getElement = Command.compileElement(element);
        const tintProps = {};
        switch (mode) {
            case 'full':
    ...
    /** 设置对话框 */
    setDialogBox({ element, properties }) {
        const getElement = Command.compileElement(element);
        const variables = [];
        const constants = [];
        for (const property of properties) {
    ...
    /** 控制对话框 */
    controlDialog({ element, operation }) {
        const method = this.controlDialog;
        if (!method.methodMap) {
            method.methodMap = {
                'pause': 'pause',
                'continue': 'continue',
    ...
    /** 设置进度条 */
    setProgressBar({ element, properties }) {
        const method = this.setProgressBar;
    ...
    /** 设置按钮 */
    setButton({ element, properties }) {
        const method = this.setButton;
        if (!method.initialized) {
            method.initialized = true;
            method.arrayMap = {
                'normalClip': true,
    ...
    /** 控制按钮 */
    controlButton({ element, operation }) {
        switch (operation) {
            case 'select-default':
                return () => {
    ...
    /** 设置动画 */
    setAnimation({ element, properties }) {
        const getElement = Command.compileElement(element);
        for (const property of properties) {
            switch (property.key) {
    ...
    /** 删除元素 */
    deleteElement({ operation, element }) {
        switch (operation) {
            case 'delete-element': {
                const getElement = Command.compileElement(element);
                return () => {
    ...
    /** 设置指针事件根元素 */
    setPointerEventRoot({ operation, element }) {
        // 补丁：2023-3-19
        if (operation === 'set') {
            operation = 'add';
        }
    ...
    /** 设置焦点 */
    setFocus({ operation, element, mode, cancelable }) {
        switch (operation) {
            case 'add': {
                // 补丁：2023-3-21
                if (mode === undefined) {
    ...
    /** 创建对象 */
    createObject({ presetId, position }) {
    ...
    /** 播放动画 */
    playAnimation({ mode, position, actor, animationId, motion, rotatable, priority, offsetY, angle, speed, wait }) {
        let getPoint;
        switch (mode) {
            case 'position':
    ...
    /** 设置对象动画 */
    setObjectAnimation({ sort, object, operation, tint, opacity, offsetY, rotation, easingId, duration, wait }) {
        let tintProps;
        let getOpacity;
        let getOffsetY;
        let getRotation;
        ...
        switch (sort) {
            case 'actor': {
                const getActor = Command.compileActor(object);
                return () => {
                    const actor = getActor();
                    if (actor) {
                        if (tintProps) {
            ...
            case 'trigger': {
                const getTrigger = Command.compileTrigger(object);
                return () => {
                    const trigger = getTrigger();
                    if (trigger) {
                        if (tintProps) {
    ...
    /** 播放音频 */
    playAudio({ type, audio, volume, location }) {
        switch (type) {
            case 'se-attenuated': {
    ...
    /** 设置音量 */
    setVolume({ type, volume, easingId, duration, wait }) {
    ...
    /** 设置声像 */
    setPan({ type, pan, easingId, duration, wait }) {
    ...
    /** 设置混响 */
    setReverb({ type, dry, wet, easingId, duration, wait }) {
    ...
    /** 创建角色 */
    createActor({ actorId, teamId, position, angle }) {
        const getPoint = Command.compilePosition(position);
    ...
    /** 移动角色 */
    moveActor({ actor, mode, angle, destination, wait }) {
        const getActor = Command.compileActor(actor);
        switch (mode) {
            case 'stop':
                return () => {
    ...
    /** 跟随角色 */
    followActor({ actor, target, mode, minDist, maxDist, offset, vertDist, bufferDist, navigate, bypass, once, wait }) {
        // 补丁：2023-1-30
        bypass = bypass ?? false;
        // 补丁：2025-3-5
    ...
    /** 平移角色 */
    translateActor({ actor, angle, distance, easingId, duration, wait }) {
        const getActor = Command.compileActor(actor);
    ...
    /** 增减仇恨值 */
    changeThreat({ actor, target, operation, threat }) {
        const method = this.changeThreat;
        ...
        const OP = method.operationMap[operation];
        const getActor = Command.compileActor(actor);
        const getTarget = Command.compileActor(target);
    ...
    /** 设置移动速度 */
    setMovementSpeed({ actor, property, base, factor }) {
        const getActor = Command.compileActor(actor);
        switch (property) {
            case 'base': {
    ...
    /** 设置角度 */
    setAngle({ actor, angle, easingId, duration, wait }) {
        const getActor = Command.compileActor(actor);
        const getAngle = Command.compileAngle(angle);
        const getDuration = Command.compileNumber(duration);
        return () => {
    ...
    /** 获取角色 */
    getActor({ variable, position, area, size, radius, selector, teamId, condition, attribute, divisor, activation, exclusion, exclusionActor, exclusionTeamId, active }) {
        // 补丁：2023-1-7
        switch (selector) {
            // @ts-ignore
            case 'team-enemy':
        ...
        if (attribute)
            attribute = Attribute.getKey(attribute);
        if (divisor)
            divisor = Attribute.getKey(divisor);
        return () => {
            const point = getPoint();
            if (point) {
    ...
    /** 获取多个角色 */
    getMultipleActors({ variable, position, area, width, height, radius, selector, teamId, activation }) {
        const setActor = Command.compileVariable(variable, Attribute.OBJECT_SET);
        const getPoint = Command.compilePosition(position);
        let getWidth;
        let getHeight;
    ...
    /** 设置队伍成员 */
    setPartyMember({ operation, actor }) {
        const getActor = Command.compileActor(actor);
        switch (operation) {
            case 'add':
                return () => {
    ...
    /** 改变通行区域 */
    changePassableTerrain({ actor, passage }) {
        const getActor = Command.compileActor(actor);
        return () => {
    ...
    /** 改变角色状态 */
    changeActorState({ actor, operation, stateId, state }) {
        const getActor = Command.compileActor(actor);
        switch (operation) {
            case 'add': {
    ...
    /** 改变角色装备 */
    changeActorEquipment({ actor, operation, slot, equipmentId, equipment }) {
        const getActor = Command.compileActor(actor);
        switch (operation) {
            case 'add': {
            ...
            case 'remove':
                return () => {
                    ...
                    if (actor) {
                        ...
                        if (equipment)
                            actor.equipment.remove(equipment);
                    }
                    return true;
                };
            case 'add-instance': {
    ...
    /** 改变角色技能 */
    changeActorSkill({ actor, operation, skillId, skill }) {
        const getActor = Command.compileActor(actor);
        switch (operation) {
            case 'add': {
    ...
    /** 改变角色头像 */
    changeActorPortrait({ actor, mode, portrait, clip }) {
        const getActor = Command.compileActor(actor);
        return () => {
            const actor = getActor();
            if (actor) {
    ...
    /** 改变角色动画 */
    changeActorAnimation({ actor, animationId }) {
        const getActor = Command.compileActor(actor);
        return () => {
    ...
    changeActorSprite({ actor, animationId, spriteId, image }) {
        const getActor = Command.compileActor(actor);
        return () => {
    ...
    /** 改变角色动作 */
    changeActorMotion({ actor, type, motion }) {
        const getActor = Command.compileActor(actor);
        const motionName = Enum.getValue(motion);
    ...
    /** 播放角色动画 */
    playActorAnimation({ actor, motion, speed, wait }) {
        const getActor = Command.compileActor(actor);
        const getSpeed = Command.compileNumber(speed, 1, 0.1, 10);
        const motionName = Enum.getValue(motion);
        return () => {
    ...
    /** 添加动画组件 */
    addAnimationComponent({ actor, animationId, motion, rotatable, syncAngle, priority, offsetY }) {
    ...
    /** 设置动画组件 */
    setAnimationComponent({ actor, animationId, motion, operation, angle, scale, speed, opacity, priority, offsetY, spriteId, image, playMotion, wait }) {
        const key = animationId + motion;
        const getActor = Command.compileActor(actor);
        switch (operation) {
            case 'set-angle': {
                ...
                return () => {
                    ...
                    if (animation)
                        animation.setAngle(getAngle(actor));
                    return true;
                };
            }
            case 'set-scale': {
    ...
    /** 移除动画组件 */
    removeAnimationComponent({ actor, animationId, motion }) {
        const key = animationId + motion;
        const getActor = Command.compileActor(actor);
        return () => {
    ...
    /** 创建全局角色 */
    createGlobalActor({ actorId, teamId }) {
        return () => {
    ...
    /** 转移全局角色 */
    transferGlobalActor({ actor, position }) {
        const getActor = Command.compileActor(actor);
        const getPoint = Command.compilePosition(position);
        return () => {
            const actor = getActor();
    ...
    /** 获取目标 */
    getTarget({ actor, selector, condition, attribute, divisor }) {
        const getActor = Command.compileActor(actor);
        switch (condition) {
            case 'max-threat':
                return () => {
    ...
    /** 探测目标 */
    detectTargets({ actor, distance, selector, inSight }) {
        const getActor = Command.compileActor(actor);
        const getDistance = Command.compileNumber(distance);
        return () => {
    ...
    /** 放弃目标 */
    discardTargets({ actor, selector, distance }) {
        const getActor = Command.compileActor(actor);
        return () => {
    ...
    /** 渲染轮廓 */
    renderOutline({ operation, actor, color }) {
        switch (operation) {
            case 'add': {
                const getActor = Command.compileActor(actor);
                const colorArray = [...Color.parseFloatArray(color)];
                return () => {
                    const actor = getActor();
                    if (actor) {
    ...
    /** 施放技能 */
    castSkill({ actor, mode, key, skillId, skill, wait }) {
        const getActor = Command.compileActor(actor);
        let getSkill;
        switch (mode) {
            case 'by-key': {
    ...
    /** 设置技能 */
    setSkill({ skill, operation, cooldown }) {
        const getSkill = Command.compileSkill(skill);
        switch (operation) {
            case 'set-cooldown': {
    ...
    /** 创建触发器 */
    createTrigger({ triggerId, caster, origin, angle, distance, scale, timeScale }) {
    ...
    /** 设置触发器持续时间 */
    setTriggerDuration({ trigger, operation, duration }) {
        const getTrigger = Command.compileTrigger(trigger);
        const getDuration = Command.compileNumber(duration);
        return () => {
            const trigger = getTrigger();
    ...
    /** 设置触发器动作 */
    setTriggerMotion({ trigger, motion }) {
        const getTrigger = Command.compileTrigger(trigger);
        const motionName = Enum.getValue(motion);
        return !motionName ? null : () => {
    ...
    /** 设置库存 */
    setInventory({ actor, operation, money, itemId, quantity, equipmentId, equipment, order1, order2, refActor }) {
        const getActor = Command.compileActor(actor);
        switch (operation) {
            case 'increase-money': {
    ...
    /** 使用物品 */
    useItem({ actor, mode, key, itemId, item, wait }) {
        const getActor = Command.compileActor(actor);
        let getItem;
        switch (mode) {
            case 'by-key': {
    ...
    /** 设置物品 */
    setItem({ item, operation, quantity }) {
        const getItem = Command.compileItem(item);
        switch (operation) {
            case 'increase': {
                const getQuantity = Command.compileNumber(quantity);
    ...
    /** 设置冷却时间 */
    setCooldown({ actor, operation, key, cooldown }) {
        const getActor = Command.compileActor(actor);
        const getKey = Command.compileEnumValue(key);
    ...
    /** 设置快捷键 */
    setShortcut({ actor, operation, key, key2, itemId, skillId }) {
        const getActor = Command.compileActor(actor);
    ...
    /** 加载场景 */
    loadScene({ sceneId, transfer, x, y }) {
    ...
    /** 加载子场景 */
    loadSubscene({ sceneId, shiftX, shiftY }) {
        const getSceneId = Command.compileString(sceneId);
        const getShiftX = Command.compileNumber(shiftX);
        const getShiftY = Command.compileNumber(shiftY);
        return () => {
            Scene.binding?.loadSubscene(getSceneId(), Math.floor(getShiftX()), Math.floor(getShiftY()));
            return true;
        };
    }
    /** 卸载子场景 */
    ...
    /** 限制摄像机边界 */
    clampCamera({ left, top, right, bottom }) {
    ...
    /** 移动摄像机 */
    moveCamera({ mode, position, actor, easingId, duration, wait }) {
        switch (mode) {
            case 'position': {
                const getPoint = Command.compilePosition(position);
                return () => {
    ...
    /** 设置缩放率 */
    setZoomFactor({ zoom, easingId, duration, wait }) {
    ...
    /** 设置环境光 */
    setAmbientLight({ red, green, blue, easingId, duration, wait }) {
    ...
    /** 改变画面色调 */
    tintScreen({ tint, easingId, duration, wait }) {
        return () => {
    ...
    /** 震动屏幕 */
    shakeScreen({ mode, power, speed, easingId, duration, wait }) {
        return () => {
    ...
    /** 设置图块 */
    setTile({ tilemap, tilemapX, tilemapY, tilesetId, tilesetX, tilesetY }) {
        ...
        return () => {
            ...
            if (tilemap) {
                tilemap.setTile(Math.floor(getTilemapX()), Math.floor(getTilemapY()), tilesetId, Math.floor(getTilesetX()), Math.floor(getTilesetY()));
            }
            return true;
        };
    }
    /** 删除图块 */
    deleteTile({ tilemap, tilemapX, tilemapY }) {
        const getTilemap = Command.compileTilemap(tilemap);
        const getTilemapX = Command.compileNumber(tilemapX, -1);
        const getTilemapY = Command.compileNumber(tilemapY, -1);
        return () => {
            getTilemap()?.deleteTile(Math.floor(getTilemapX()), Math.floor(getTilemapY()));
            return true;
        };
    }
    /** 设置地形 */
    ...
    /** 设置游戏速度 */
    setGameSpeed({ speed, easingId, duration, wait }) {
    ...
    /** 设置队伍关系 */
    setTeamRelation({ teamId1, teamId2, relation }) {
        return () => {
    ...
    /** 开关碰撞系统 */
    switchCollisionSystem({ operation }) {
        switch (operation) {
    ...
    /** 游戏数据 */
    gameData({ operation, index, variables }) {
        switch (operation) {
            case 'save': {
