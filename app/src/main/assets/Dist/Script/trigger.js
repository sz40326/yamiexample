...
class Trigger {
    ...
    updateCollision() {
        ...
        if (this.elapsed < this.initialDelay || this.elapsed >= this.timeout)
            return true;
    ...
    static latest;
    /** 临时角色列表 */
    static actors = CacheList.instance;
    /** 是否与角色形状发生碰撞 */
    ...
    // 触发器角色获取器
    static actorGetters = new class ActorGetters {
        /**
         * 获取矩形碰撞区域中的角色
         * @param x 触发器位置X
         * @param y 触发器位置Y
        ...
        'rectangle' = (x, y, angle, scale, shape) => {
            ...
            // 获取矩形触发区域所在的角色分区列表
            const cells = Scene.actor.partition.get(tl - expansion, tt - expansion, tr + expansion, tb + expansion);
        ...
        'circle' = (x, y, angle, scale, shape) => {
            ...
            const expansion = Trigger.collideWithActorShape ? Scene.binding.maxColliderHalf : 0;
            // 获取圆形触发区域所在的角色分区列表
            const cells = Scene.actor.partition.get(x - radius - expansion, y - radius - expansion, x + radius + expansion, y + radius + expansion);
            const length = cells.count;
            for (let i = 0; i < length; i++) {
                const actors = cells[i];
                const length = actors.length;
    ...
    // 检测与墙壁的碰撞
    static detectCollisionWithWalls = new class DetectCollisionWithWalls {
        /**
         * 检测与墙壁的碰撞 - 穿透
         * @returns 不发生碰撞
         */
    ...
    // 碰撞过滤器
    static collisionFilters = new class CollisionFilters {
        /**
         * 碰撞过滤器 - 一次
         * @returns 角色缓存列表
         */
    ...
    // 击中列表更新器
    static hitListUpdaters = new class HitListUpdaters {
        /** 击中列表更新器 - 一次 */
        'once' = Function.empty;
        /** 击中列表更新器 - 碰撞期间一次 */
