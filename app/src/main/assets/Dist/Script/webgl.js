...
let GLBuilder = new class WebGLBuilder {
    ...
    textureOptions = {
        /** 纹理放大滤波器 */
        magFilter: WebGLRenderingContext.prototype.NEAREST,
        /** 纹理缩小滤波器 */
        minFilter: WebGLRenderingContext.prototype.LINEAR,
        /** 纹理坐标水平填充 */
     ...
     */
    createWebGL2(canvas) {
        return canvas.getContext('webgl2', this.webglOptions);
    }
    /**
     * 创建WebGL1上下文
     * @param canvas 画布元素
     */
    createWebGL1(canvas) {
        const webgl = canvas.getContext('webgl', this.webglOptions);
        if (!webgl) {
        ...
        WebGLRenderingContext.prototype.bufferData = function (target, srcData, usage, srcOffset, length) {
            if (length !== undefined) {
    ...
    setupWebGLProperties(GL) {
        ...
        // 创建遮罩纹理
        GL.maskTexture = new Texture({ format: GL.RGBA });
        ...
        GL.offscreen = {
            enabled: false,
            current: new Texture({ format: GL.RGB }),
            last: new Texture({ format: GL.RGB }),
        };
        const { current, last } = GL.offscreen;
        current.fbo = GL.createTextureFBO(current);
        last.fbo = GL.createTextureFBO(last);
        // 创建程序对象
        GL.imageProgram = GL.createImageProgram();
        GL.tileProgram = GL.createTileProgram();
        GL.spriteProgram = GL.createSpriteProgram();
        GL.particleProgram = GL.createParticleProgram();
        GL.lightProgram = GL.createLightProgram();
        GL.graphicProgram = GL.createGraphicProgram();
    }
    /**
     * 安装WebGL方法
     * @param GL 上下文对象
     */
    setupWebGLMethods(GL) {
        // WebGL上下文方法 - 创建程序对象
        ...
        // WebGL上下文方法 - 设置环境光
        GL.setAmbientLight = function (light) {
        ...
        namespace: {
            ...
            GL.drawImage = function (texture, dx, dy, dw, dh, tint = defTint) {
                ...
                // 计算变换矩阵
                const matrix = Matrix.instance.project(this.flip, this.width, this.height).multiply(this.matrix);
                // 计算顶点数据
                const dl = dx + 0.004;
                const dt = dy + 0.004;
                const dr = dl + dw;
...
class ImageTexture extends Texture {
    ...
    updateSliceData(width, height, clip, border) {
        ...
        Array.fill(this.sliceClip, clip);
     ...
     */
    on(type, callback) {
        BaseTexture.prototype.on.call(this, type, callback);
    }
    /**
     * 执行加载回调
     * @param type 回调事件类型
