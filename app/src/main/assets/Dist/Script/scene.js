...
let Scene = new class SceneManager {
    ...
    initialize() {
        ...
        // 创建精灵渲染器
        this.spriteRenderer = new SceneSpriteRenderer(this.visibleActors = new CacheList(), this.visibleAnimations = new CacheList(), this.visibleTriggers = new CacheList(), this.visibleEmitters = new CacheList());
        // 创建直射光渲染器
        ...
        Input.on('keydown', (event) => Scene.emitInputEvent('keydown', event));
        Input.on('keyup', (event) => Scene.emitInputEvent('keyup', event));
        Input.on('mousedown', (event) => Scene.emitInputEvent('mousedown', event));
        Input.on('mouseup', (event) => Scene.emitInputEvent('mouseup', event));
        Input.on('mousemove', (event) => Scene.emitInputEvent('mousemove', event));
        Input.on('doubleclick', (event) => Scene.emitInputEvent('doubleclick', event));
        Input.on('wheel', (event) => Scene.emitInputEvent('wheel', event));
        Input.on('touchstart', (event) => Scene.emitInputEvent('touchstart', event));
        Input.on('touchmove', (event) => Scene.emitInputEvent('touchmove', event));
        Input.on('touchend', (event) => Scene.emitInputEvent('touchend', event));
        Input.on('gamepadbuttonpress', (event) => Scene.emitInputEvent('gamepadbuttonpress', event));
        Input.on('gamepadbuttonrelease', (event) => Scene.emitInputEvent('gamepadbuttonrelease', event));
        Input.on('gamepadleftstickchange', (event) => Scene.emitInputEvent('gamepadleftstickchange', event));
        Input.on('gamepadrightstickchange', (event) => Scene.emitInputEvent('gamepadrightstickchange', event));
    }
    /** 加载预设对象 */
    loadPresets() {
        const presets = this.presets;
        // 加载场景对象目录
        const load = (nodes, scene) => {
    ...
    getActorAt(x, y) {
        ...
        const expansion = Math.max(Math.abs(selection.top), Math.abs(selection.left), Math.abs(selection.right), Math.abs(selection.bottom));
        selection.x = x;
        selection.y = y;
    ...
    bind(scene) {
        ...
        this.spriteRenderer.setObjectLists(scene.actor.list, scene.animation.list, scene.trigger.list, scene.emitter.list);
        // 初始化场景
...
class SceneContext {
    ...
    constructor(data = {}, savedData) {
        ...
        // 设置更新器
        this.updaters = new UpdaterList(this.parallax, this.actor, this.animation, this.trigger, this.region, this.light, Camera, this.emitter);
        // 设置渲染器
        this.renderers = new RendererList(this.light, this.parallax.backgrounds, Scene.spriteRenderer, Scene.directLightRenderer, this.parallax.foregrounds);
     ...
     * @returns 目标角色
     */
    getActor({ x, y, area = 'square', size = 1, radius = 0.5, selector = 'any', teamId = '', condition = 'nearest', attribute = '', divisor = '', activation = 'active', exclusionActor = undefined, exclusionTeamId = '', }) {
        const inspector = SceneContext.actorInspectors[selector];
        const teamIndex = Team.get(teamId)?.index ?? -1;
        const actorPool = CacheList.instance;
        let count = 0;
        let skipCond;
        switch (activation) {
            case 'active':
     ...
     * @returns 目标角色列表
     */
    getMultipleActors({ x, y, area = 'rectangle', width = 1, height = 1, radius = 0.5, selector = 'any', teamId = '', activation = 'active', }) {
        const inspector = SceneContext.actorInspectors[selector];
        const teamIndex = Team.get(teamId)?.index ?? -1;
        const actors = [];
        let skipCond;
     ...
     */
    setAmbientLight(red, green, blue, easingId = '', duration = 0) {
    ...
    // 角色检查器集合
    static actorInspectors = new class ActorInspectors {
        /**
         * 检查器 - 判断敌对角色
    ...
    // 角色过滤器集合
    static actorFilters = new class ActorFilters {
        /**
         * 角色过滤器 - 最近距离
         * @param x 场景位置X
         * @param y 场景位置Y
         ...
         * @returns 目标角色
         */
        'min-attribute-value' = (x, y, attribute) => {
         ...
         * @returns 目标角色
         */
        'max-attribute-value' = (x, y, attribute) => {
         ...
         * @returns 目标角色
         */
        'min-attribute-ratio' = (x, y, attribute, divisor) => {
         ...
         * @returns 目标角色
         */
        'max-attribute-ratio' = (x, y, attribute, divisor) => {
...
class SceneTerrainManager {
    ...
    get(x, y, z = 1) {
        ...
        if (x >= 0 && x < width && y >= 0 && y < height) {
            return this.arrays[z][x + y * width];
        }
        return -1;
    }
    /**
...
class SceneTilemap {
        ...
        for (let y = top; y < bottom; y++) {
            for (let x = left; x < right; x++) {
                if (x >= 0 && x < width && y >= 0 && y < height) {
                    ...
                    const r = width - 1;
                    const b = height - 1;
                    const neighbor = (x > 0 && key !== tiles[ti - 1] >> 8) + 1
                    ...
                    for (let i = 0; i < length; i++) {
                        ...
                        if (Math.max(code & 0b11, code >> 2 & 0b11, code >> 4 & 0b11, code >> 6 & 0b11, code >> 8 & 0b11, code >> 10 & 0b11, code >> 12 & 0b11, code >> 14 & 0b11) !== 0b11) {
                            nodeIndex = i;
                            break;
                        }
                    }
    ...
    draw() {
        ...
        if (endIndex !== 0) {
            ...
            const lightModeIndex = modeMap[lightMode];
            const matrix = Matrix.instance.project(gl.flip, Camera.width, Camera.height).translate(ox - sl, oy - st);
...
class ScenePartitionManager {
    ...
    update(object) {
        ...
        if (object.cellId !== cellId) {
            // 从旧的分区中删除对象
            if (object.cellId !== -1) {
                this.cells[object.cellId].remove(object);
            }
            // 添加对象到新的分区
            if (cellId !== -1) {
                this.cells[cellId].push(object);
            }
    ...
    saveData() {
        ...
        for (let i = 0; i < length; i++) {
            ...
            if (!actor.active)
                continue;
            // 保存全局角色或普通角色
            data.push(actor instanceof GlobalActor
...
class SceneLightManager {
    ...
    render() {
        ...
        // 绘制反射光
        if (qi !== 0) {
            const projMatrix = Matrix.instance.project(gl.flip, sr - sl, sb - st)
                .translate(-sl, -st)
                .scale(tw, th);
            // 按队列顺序绘制所有可见光源
...
class SceneLight {
     ...
     * @param duration 持续时间(毫秒)
     */
    move(properties, easingId = '', duration = 0) {
        // 转换属性词条的数据结构
...
class SceneSpriteRenderer {
    ...
    render() {
        ...
        for (let i = 0; i < dLength; i++) {
            ...
            for (let y = by; y < ey; y++) {
                for (let x = bx; x < ex; x++) {
                    ...
                    const anchor = (round(max(min(px, 1), 0) * 0xffff)
        ...
        if (li !== 0) {
            ...
            const matrix = gl.matrix.project(gl.flip, Camera.width, Camera.height).translate(-sl, -st);
            // 绑定渲染器程序为当前程序
            // 切换成粒子程序后自动恢复
