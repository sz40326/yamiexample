...
class Printer {
    ...
    measureHeight(text) {
        if (this.horizontal) {
            ...
            for (let i = 0; i < length; i++) {
                height = Math.max(height, context.measureText(text[i]).width);
            }
            return height;
        }
    }
    ...
    draw(content) {
        ...
        while (this.index < length) {
            ...
            if (wordWrap && (horizontal
                ...
                : this.y + Printer.lineWidth + (charWidth = this.measureWidth(char)) > printHeight) && (this.breakable || this.buffer.length !== 0)) {
                this.drawBuffer();
                this.newLine();
                continue;
            }
    ...
    matchTag() {
        ...
        if (match = string.match(regexps.textPosition)) {
            ...
            this.drawBuffer();
            const position = (operation === 'set' ? value
    ...
    static async initialize() {
        // 设置字符宽度数组
        this.charWidths = new Float64Array(GL.arrays[1].uint32.buffer, 0, GL.arrays[1].uint32.length / 2);
        // 创建打印机指令列表
     ...
     */
    static async importFonts(imports) {
...
/** ******************************** 打印机图像列表 ******************************** */
class PrinterImageList extends Array {
    /** 是否已改变 */
    changed = false;
}
