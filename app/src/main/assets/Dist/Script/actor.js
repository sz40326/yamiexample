...
class Actor {
     ...
     */
    translate(angle, distance, easingId = '', duration = 0, key = 'translate') {
     ...
     * @param duration 持续时间(毫秒)
     */
    setScale(scale, easingId = '', duration = 0) {
        if (duration > 0) {
            // 创建过渡更新器，使用set方法:
            // 如果已有同名更新器，则替换
            let elapsed = 0;
     ...
     * @param duration 持续时间(毫秒)
     */
    setAngle(angle, easingId = '', duration = 0) {
        if (duration > 0) {
     ...
     */
    rotate(angle, easingId = '', duration = 0, key = 'rotate') {
        if (duration > 0) {
            // 创建过渡更新器，使用set方法:
            // 如果已有同名更新器，则替换
            let elapsed = 0;
     ...
     * @param duration 持续时间(毫秒)
     */
    setTintForAll(tint, easingId = '', duration = 0) {
     ...
     * @param duration 持续时间(毫秒)
     */
    setOpacity(opacity, easingId = '', duration = 0) {
     ...
     * @param duration 持续时间(毫秒)
     */
    setOpacityForAll(opacity, easingId = '', duration = 0) {
        for (const animation of this.animationManager.list) {
     ...
     * @param duration 持续时间(毫秒)
     */
    setOffsetY(offsetY, easingId = '', duration = 0) {
     ...
     * @param duration 持续时间(毫秒)
     */
    setOffsetYForAll(offsetY, easingId = '', duration = 0) {
        for (const animation of this.animationManager.list) {
     ...
     * @param duration 持续时间(毫秒)
     */
    setRotation(rotation, easingId = '', duration = 0) {
     ...
     * @param duration 持续时间(毫秒)
     */
    setRotationForAll(rotation, easingId = '', duration = 0) {
        for (const animation of this.animationManager.list) {
    ...
    /** 定义临时属性映射表函数 */
    static defineTempAttributes(attributes) { }
    /** 初始化角色相关的数据 */
    static initialize() {
        // 创建角色临时属性的描述器
    ...
    /** 角色检查器集合 */
    static inspectors = new class ActorInspectors {
        /** 检查器 - 判断敌对角色 */
        'enemy' = (a, b) => {
...
class GlobalActor extends Actor {
    /** 场景角色的本地事件标记键 */
    static localEventKey = Symbol("LOCAL_EVENT");
    /**
     * 转移到场景中的指定位置
     * @param x 场景坐标X
     * @param y 场景坐标Y
    ...
    callEvent(type) {
        ...
        if (event && this.presetId !== '') {
            ...
            if (this.events.hasOwnProperty(type) && event.initial === this.events[type]) {
                event[GlobalActor.localEventKey] = true;
            }
        }
        return event;
    }
    ...
    setEventMap(map) {
        for (const updater of this.updaters) {
            if (updater instanceof EventHandler &&
                GlobalActor.localEventKey in updater) {
                updater.finish();
            }
        }
...
class ActorCollider {
    ...
    handleImmovableCollisions() {
        ...
        // 获取探测范围所在的角色区间列表
        const cells = Scene.actor.partition.get(ox - half - expansion, oy - half - expansion, ox + half + expansion, oy + half + expansion);
    ...
    static handleSceneCollisions() {
        if (ActorCollider.sceneCollisionEnabled === false)
            return;
        ...
        for (let i = 0; i < length; i++) {
            ...
            if (actor.y < radius)
                actor.y = radius;
            if (actor.x > width - radius)
                actor.x = width - radius;
            if (actor.y > height - radius)
                actor.y = height - radius;
    ...
    static handleActorCollisions() {
        if (ActorCollider.actorCollisionEnabled === false)
            return;
    ...
    static handleCollisionBetweenTwoActors = (IIFE => {
        ...
        // 碰撞 - 正方形和正方形
        const collideSquareAndSquare = (sCollider, dCollider, ratio) => {
        ...
        // 碰撞 - 圆形和圆形
        const collideCircleAndCircle = (sCollider, dCollider, ratio) => {
            const sActor = sCollider.actor;
            const dActor = dCollider.actor;
            const distMin = sCollider.half + dCollider.half;
        ...
        // 碰撞 - 正方形和圆形
        const collideSquareAndCircle = (sCollider, dCollider, ratio) => {
            const sActor = sCollider.actor;
            const dActor = dCollider.actor;
...
class ActorNavigator {
     ...
     */
    followCircle(target, minDist, maxDist, offset = 0, bufferDist = 0, navigate = false, bypass = false, once = false) {
        if (this.mode !== 'follow') {
            this.stopMoving();
            this.mode = 'follow';
        }
     ...
     * @param once 跟随一次(到达位置后停止移动)
     */
    followRectangle(target, minDist, maxDist, vertDist = 0, bufferDist = 0, navigate = false, bypass = false, once = false) {
        if (this.mode !== 'follow') {
            this.stopMoving();
            this.mode = 'follow';
        }
    ...
    _circleFollowTarget(deltaTime) {
        ...
        const dist = Math.sqrt((actor.x - target.x) ** 2
            + (actor.y - target.y) ** 2);
        // 如果角色距离大于最大距离，开始接近
        // 设置更新函数为：接近目标(圆形模式)
...
class AnimationController {
    ...
    startIdle() {
        ...
        if (this.animation && this.playing === false) {
            if (this.animation.motionName === this.idleMotion)
                return;
    ...
    startMoving() {
        this.state = 'move';
        if (this.animation && this.playing === false) {
            if (this.animation.motionName === this.moveMotion)
                return;
...
class SkillManager {
    ...
    sort() {
        ...
        const list = Object.values(this.idMap).sort((a, b) => a.data.filename.localeCompare(b.data.filename));
        // 遍历技能列表，重构idMap
        const length = list.length;
        for (let i = 0; i < length; i++) {
            const skill = list[i];
...
class EquipmentManager {
     ...
     */
    saveData() {
        const equipments = Object.values(this.slotMap);
        const length = equipments.length;
...
class ShortcutManager {
     ...
     */
    saveData() {
        const shortcuts = Object.values(this.keyMap);
...
class TargetManager {
     ...
     * @returns 是否已经存在目标
     */
    detect(distance, selector, inSight = false) {
        const inspector = Actor.inspectors[selector];
        const owner = this.actor;
        const ox = owner.x;
        const oy = owner.y;
        // 获取探测范围所在的角色区间列表
        const cells = Scene.actor.partition.get(ox - distance, oy - distance, ox + distance, oy + distance);
     ...
     * @returns 目标池中符合条件的角色实例
     */
    getTargetMinAttributeValue(selector, key) {
        let target;
        let weight = Infinity;
        const inspector = Actor.inspectors[selector];
        const owner = this.actor;
     ...
     * @returns 目标池中符合条件的角色实例
     */
    getTargetMaxAttributeValue(selector, key) {
        let target;
        let weight = -Infinity;
        const inspector = Actor.inspectors[selector];
        const owner = this.actor;
     ...
     * @returns 目标池中符合条件的角色实例
     */
    getTargetMinAttributeRatio(selector, key, divisor) {
        let target;
        let weight = Infinity;
        const inspector = Actor.inspectors[selector];
        const owner = this.actor;
     ...
     * @returns 目标池中符合条件的角色实例
     */
    getTargetMaxAttributeRatio(selector, key, divisor) {
        let target;
        let weight = -Infinity;
        const inspector = Actor.inspectors[selector];
        const owner = this.actor;
