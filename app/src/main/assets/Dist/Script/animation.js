...
class AnimationPlayer {
     ...
     */
    setTint(id, updaters, tint, easingId = '', duration = 0) {
        const { red, green, blue, gray } = tint;
        // 优先使用现有的色调数组
        let activeTint = this.tints.find((t) => t.key === this);
        // 创建动画色调数组
        if (!activeTint) {
            activeTint = [0, 0, 0, 0];
            activeTint.key = this;
     ...
     * @param duration 持续时间(毫秒)
     */
    setOpacity(id, updaters, opacity, easingId = '', duration = 0) {
        if (duration > 0) {
            let elapsed = 0;
     ...
     * @param duration 持续时间(毫秒)
     */
    setOffsetY(id, updaters, offsetY, easingId = '', duration = 0) {
        if (duration > 0) {
            let elapsed = 0;
     ...
     * @param duration 持续时间(毫秒)
     */
    setRotation(id, updaters, rotation, easingId = '', duration = 0) {
        if (duration > 0) {
            let elapsed = 0;
            ...
            for (let i = 0; i <= last; i++) {
                ...
                if (index >= start && index < end) {
                    ...
                    if (easingId && i < last) {
                        ...
                        const time = Easing.get(easingId).get((index - start) / (next.start - start));
                        // 更新插值后的上下文
    ...
    drawSprite(context, texture, light) {
        ...
        const anchor = light !== 'anchor' ? 0 : (Math.round(Math.clamp(this.anchorX, 0, 1) * 0xffff)
     ...
     * @param matrix 动画矩阵
     */
    drawUIAnimation(x, y, opacity, matrix) {
        GL.alpha = opacity;
    ...
    static contextUpdateParticle(frame, time, next) {
        ...
        if (this.emitter === null) {
            ...
            const emitter = new SceneParticleEmitter(data);
...
class ParticleLayer {
    ...
    draw(matrix) {
        ...
        if (vi !== 0) {
            ...
            const vertices = gl.arrays[0].float32;
            if (matrix === undefined) {
                matrix = gl.matrix.project(gl.flip, gl.width, gl.height)
    ...
    /** 共享32位整型数组A */
    static sharedUint32A;
    /** 共享32位整型数组B */
    static sharedUint32B;
    /** 光线采样模式映射表 */
...
class ParticleElement {
     ...
     * @returns 生成的参数值
     */
    static getRandomParameter(parameter) {
